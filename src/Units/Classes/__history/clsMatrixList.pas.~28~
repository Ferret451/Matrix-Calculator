...
type
  ...
  public
    ...
    procedure Sort(const AOrder: Boolean);
...
procedure TMatrixList<T>.Sort(const AOrder: Boolean);
var
...
begin
  ...
  begin
    ...
    while CurrNode <> BarrierNode do
    begin
      if ((CurrNode^.FValue.Name > CurrNode^.FNext^.FValue.Name) and AOrder) or
        ((CurrNode^.FValue.Name < CurrNode^.FNext^.FValue.Name) and not AOrder) then
      begin
